...
function GPTImageNode({ data, selected }: NodeProps<NodeData>) {
  ...
  return (
    <div className={`node-container ${selected ? 'selected' : ''} ${status === 'running' ? 'node-running' : ''}`} style={{ borderColor: '#fd79a8' }}>
      ...
      <div className="node-body">
        <div className="flex justify-between text-[10px]">
          <span>{data.gptImageAspectRatio || '1:1'}</span>
          <span className="text-text-secondary">{(data.gptImageQuality as string) !== 'auto' && (data.gptImageQuality as string) !== undefined ? `· ${data.gptImageQuality}` : ''}</span>
        </div>
