  ...
  const handleExportConfig = () => {
    ...
    for (const [id, provider] of Object.entries(providers)) {
      if (exportIncludeKeys) {
        exportData[id] = JSON.parse(JSON.stringify(provider))
      } else {
    ...
    a.href = url
    a.download = `nbc_provider_config_${new Date().toISOString().slice(0, 10)}.json`
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const { providers: currentProviders, updateProvider } = useProviderStore.getState()
        ...
        localStorage.setItem('nbc_provider_store', JSON.stringify(merged))
        useNotificationStore.getState().addNotification({
          type: 'success',
          title: '配置已导入',
          message: `成功导入 ${Object.keys(imported).length} 个 Provider 配置。`
        })
      } catch (err) {
        alert('导入失败：文件格式不正确。')
      }
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40" onClick={() => setExportModalOpen(false)}>
          <div className="bg-bg-primary border border-node-border rounded-xl p-6 w-96 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold mb-4 flex items-center gap-2">
              <Download size={16} /> 导出 Provider 配置
            </h3>
            <p className="text-xs text-text-secondary mb-4">
              导出的 JSON 文件可用于备份或分享给你的团队成员。导入时，已有的相同 ID 的 Provider 会合并更新。
            </p>
