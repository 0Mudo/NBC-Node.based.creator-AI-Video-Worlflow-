...
const TYPE_LABELS: Record<string, string> = {
  gptImage2: 'GPT图像', seedance: 'Seedance视频', banana: 'Banana图像'
}
...
export async function executeNode(nodeId: string) {
  ...
  updateNodeData(nodeId, { _negativePrompt: negativePrompt || undefined, _consistencySeed: consistencySeed })
...
export async function executeAll() {
  ...
  const genTypes = new Set(['gptImage2', 'seedance', 'banana'])
