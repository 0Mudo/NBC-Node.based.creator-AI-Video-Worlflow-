...
interface AssetStore {
  ...
  updateAssetTags: (id: string, tags: AssetTag[]) => void
  updateAssetProject: (id: string, projectId: string | undefined) => void
  ...
  persist(
    (set) => ({
      ...
      removeAssets: (ids) => set((state) => {
        ...
        const filtered = state.assets.filter((a) => !idSet.has(a.id))
        return {
          assets: filtered,
          selectedAssetId: state.selectedAssetId && idSet.has(state.selectedAssetId) ? null : state.selectedAssetId,
          selectedAssetIds: state.selectedAssetIds.filter(sid => !idSet.has(sid)),
        }
      }),
      ...
      updateAssetTags: (id, tags) => set((state) => ({
        assets: state.assets.map((a) => a.id === id ? { ...a, tags } : a),
      })),
      updateAssetProject: (id, projectId) => set((state) => ({
        assets: state.assets.map((a) => a.id === id ? { ...a, projectId } : a),
      })),
      toggleMultiSelectMode: () => set((state) => ({
        isMultiSelectMode: !state.isMultiSelectMode,
        selectedAssetIds: state.isMultiSelectMode ? [] : state.selectedAssetIds,
      })),
      ...
      selectAllFiltered: (ids) => set({ selectedAssetIds: ids }),
      clearSelection: () => set({ selectedAssetIds: [], isMultiSelectMode: false }),
    }),
    {
